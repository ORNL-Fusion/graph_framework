...
void test_sqrt() {
    ...
    assert(sqrt_ten->evaluate().at(0) == std::sqrt(backend::base_cast<BACKEND> (10.0)));
    ...
    assert(sqrt_var->evaluate().at(0) == std::sqrt(backend::base_cast<BACKEND> (3.0)));
    ...
    assert(sqrt_var_vec_result.at(0) == std::sqrt(backend::base_cast<BACKEND> (4.0)));
    assert(sqrt_var_vec_result.at(1) == std::sqrt(backend::base_cast<BACKEND> (7.0)));
...
//  d sqrt(x) / dx = 1/(2 Sqrt(x))
    ...
    assert(dsqrt_var->evaluate().at(0) ==
           ...
           std::sqrt(backend::base_cast<BACKEND> (3.0)) &&
           "Expected 0.5*sqrt(3)");
...
void test_exp() {
    ...
    assert(exp_ten->evaluate().at(0) == std::exp(backend::base_cast<BACKEND> (10.0)));
#ifdef USE_REDUCE
    ...
    var->set(backend::base_cast<BACKEND> (3.0));
    assert(exp_var->evaluate().at(0) == std::exp(backend::base_cast<BACKEND> (3.0)));
    ...
    assert(exp_var_vec_result.at(0) == std::exp(backend::base_cast<BACKEND> (4.0)));
    assert(exp_var_vec_result.at(1) == std::exp(backend::base_cast<BACKEND> (7.0)));
...
#endif
    assert(dexp_var->evaluate().at(0) == std::exp(backend::base_cast<BACKEND> (3.0)));
}
...
//  a^1 = a
    ...
    assert(graph::pow_cast(graph::pow(ten, one)).get() &&
           "Expected a pow node.");
#endif
    assert(graph::pow(ten, one)->evaluate().at(0) == backend::base_cast<BACKEND> (10.0) &&
           "Expected ten.");
...
//  Sqrt(a)^2 = a
    ...
    assert(graph::pow(graph::sqrt(ten), two)->is_match(ten) &&
           "Expected ten.");
    assert(graph::pow(graph::sqrt(ten), two)->evaluate().at(0) == backend::base_cast<BACKEND> (10.0) &&
           "Expected ten.");
#else
    assert(graph::pow_cast(graph::pow(graph::sqrt(ten), two)).get() &&
           "Expected a pow node.");
    assert(graph::pow(graph::sqrt(ten), two)->evaluate().at(0) == backend::base_cast<BACKEND> (std::pow(std::sqrt(10.0), 2.0)) &&
           "Expected ten.");
#endif
...
void test_log() {
    ...
    assert(graph::constant_cast(graph::log(graph::constant<BACKEND> (10))) &&
           "Expected constant");
#else
    assert(graph::log_cast(graph::log(graph::constant<BACKEND> (10))) &&
           "Expected log node");
#endif

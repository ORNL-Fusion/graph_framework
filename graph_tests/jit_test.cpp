...
//------------------------------------------------------------------------------
template<typename T> void compile(graph::input_nodes<backend::cpu<T>> inputs,
                                  graph::output_nodes<backend::cpu<T>> outputs,
                                  graph::map_nodes<backend::cpu<T>> setters,
                                  const T expected,
                                  const T tolarance) {
    jit::kernel<backend::cpu<T>> source("test_kernel", inputs, outputs, setters);
...
template<typename T> void run_math_tests() {
    auto v1 = graph::variable<backend::cpu<T>> (1, "v1");
    auto v2 = graph::variable<backend::cpu<T>> (1, "v2");
    auto v3 = graph::variable<backend::cpu<T>> (1, "v3");
    ...
    auto fma_node_dfdv4 = fma_node->df(graph::variable<backend::cpu<T>> (1, ""));
    compile<T> ({graph::variable_cast(v1),
                 graph::variable_cast(v2),
                 graph::variable_cast(v3)},
                {fma_node_dfdv4}, {},
    ...
    auto v4 = graph::variable<backend::cpu<T>> (1, "v4");
...
void run_dispersion_test(equilibrium::unique_equilibrium<typename DISPERSION_FUNCTION::backend> &eq,
                         const typename DISPERSION_FUNCTION::backend::T tolarance) {
    ...
    w->set(static_cast<typename DISPERSION_FUNCTION::T> (1.0));
    x->set(static_cast<typename DISPERSION_FUNCTION::T> (1.0));
    y->set(static_cast<typename DISPERSION_FUNCTION::T> (1.0));
    z->set(static_cast<typename DISPERSION_FUNCTION::T> (1.0));
    kx->set(static_cast<typename DISPERSION_FUNCTION::T> (1.0));
    ky->set(static_cast<typename DISPERSION_FUNCTION::T> (1.0));
    kz->set(static_cast<typename DISPERSION_FUNCTION::T> (1.0));
    t->set(static_cast<typename DISPERSION_FUNCTION::T> (1.0));

    dispersion::dispersion_interface<DISPERSION_FUNCTION> D(w, kx, ky, kz, x, y, z, t, eq);
    auto residule = D.get_d();

    compile<typename DISPERSION_FUNCTION::backend::T> ({graph::variable_cast(w),
...
void run_dispersion_tests() {
    auto no_mag_eq = equilibrium::make_no_magnetic_field<backend::cpu<T>> ();

    run_dispersion_test<dispersion::bohm_gross<backend::cpu<T>>> (no_mag_eq, 0.0);

    auto slab_eq = equilibrium::make_no_magnetic_field<backend::cpu<T>> ();

    run_dispersion_test<dispersion::simple<backend::cpu<T>>> (slab_eq, 0.0);
    run_dispersion_test<dispersion::ordinary_wave<backend::cpu<T>>> (slab_eq, 0.0);
    run_dispersion_test<dispersion::extra_ordinary_wave<backend::cpu<T>>> (slab_eq,
#ifdef USE_CUDA
                                                                              0.032);
#else
                                                                              0.0);
#endif
    run_dispersion_test<dispersion::cold_plasma<backend::cpu<T>>> (slab_eq,
#ifdef USE_CUDA
                                                                      1.4E10);
#else
                                                                      5.0E9);
...
template<typename T> void run_tests() {
    if constexpr (jit::can_jit<backend::cpu<T>> ()) {
        run_math_tests<T> ();

...
void test_efit() {
    ...
    std::mutex sync;
    auto eq = equilibrium::efit_equilibrium<T> ("/Users/m4c/efit.nc",
                                                x_var, y_var, z_var, sync);
...
template<typename T> void sherwood() {
    auto m = graph::constant(static_cast<T> (10.0));
    auto b = graph::constant(static_cast<T> (-3.0));
    auto x = graph::variable<T> (4, "x");
    auto y = m*x + b;
    ...
    graph::input_nodes<T> inputs = {graph::variable_cast(x)};
    graph::output_nodes<T> outputs = {y, dydx};
    graph::map_nodes<T> maps = {};
    source.add_kernel("sherwood_kernel",
                      inputs, outputs, maps);
    source.compile();
    
    inputs[0]->set(std::vector<T> ({1.0, 2.0, 3.0, 4.0}));
    
    auto run = source.create_kernel_call("sherwood_kernel",
                                         inputs,
                                         outputs,
                                         inputs.back()->size());
    source.print_source();

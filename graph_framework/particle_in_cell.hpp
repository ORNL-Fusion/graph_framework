...
//------------------------------------------------------------------------------
    ...
    class apply_u_node final : public no_derivative<T, false,
                                                    n_branch_node<7, T, false>,
                                                    7> {
    private:
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
        static std::string to_string(std::array<shared_leaf<T>, 7> branches) {
        ...
        static void func(backend::buffer<T> &x,
            ...
            for (size_t j = 0; j < size; j++) {
                ...
                if constexpr (std::same_as<T, float>) {
                    uint32_t rand_j = std::bit_cast<uint32_t> (rand[j]);
                    for (uint8_t k = 0, ke = i[j]; k < ke; k++, rand_j >>= 1) {
                        ...
                        temp_x = (E0*a_j + b_j + C)/mof_j;
                    }
                } else {
                    uint64_t rand_j = std::bit_cast<uint64_t> (rand[j]);
                    for (uint8_t k = 0, ke = i[j]; k < ke; k++, rand_j >>= 1) {
                        const T E0 = mof_j*temp_x;
                        const int8_t rm = 4*(rand_j & 1) - 2;
                        const T C = rm*std::sqrt(tbnu_e_dt_j*E0);
...
///  @param[in] B         B collision factor.
//------------------------------------------------------------------------------
        apply_u_node(shared_leaf<T> x,
                     shared_leaf<T> i,
                     shared_leaf<T> rand,
                     shared_leaf<T> mof,
                     shared_leaf<T> tbnu_e_dt,
                     shared_leaf<T> A,
                     shared_leaf<T> B) :
        no_derivative<T, false,
                      n_branch_node<7, T, false>, 7> ({x, i, rand, mof, tbnu_e_dt, A, B},
...
//------------------------------------------------------------------------------
        virtual void compile_preamble(std::ostringstream &stream,
                                      ...
                                      int &avail_const_mem) {
            if (!pre_funcs.contains("apply_u")) {
                pre_funcs.insert("apply_u");
...
//------------------------------------------------------------------------------
    template<std::floating_point T>
    shared_leaf<T> apply_u(shared_leaf<T> x,
                           shared_leaf<T> i,
                           shared_leaf<T> rand,
                           shared_leaf<T> mof,
                           shared_leaf<T> tbnu_e_dt,
                           shared_leaf<T> A,
                           shared_leaf<T> B) {
        auto temp = std::make_shared<apply_u_node<T>> (x, i, rand, mof,
                                                       tbnu_e_dt, A, B)->reduce();
//  Test for hash collisions.
        for (size_t i = temp->get_hash();
             i < std::numeric_limits<size_t>::max(); i++) {
            if (leaf_node<T>::caches.nodes.find(i) ==
                leaf_node<T>::caches.nodes.end()) {
                leaf_node<T>::caches.nodes[i] = temp;
                return temp;
            } else if (temp->is_match(leaf_node<T>::caches.nodes[i])) {
                return leaf_node<T>::caches.nodes[i];
            }
        }
...
//------------------------------------------------------------------------------
    template<std::floating_point T>
    shared_apply_u<T> apply_u_cast(shared_leaf<T> x) {
...
//------------------------------------------------------------------------------
    template<std::floating_point T>
    class apply_xi_node final : public no_derivative<T, false,
                                                     n_branch_node<4, T, false>,
                                                     4> {
    private:
//------------------------------------------------------------------------------
///  @brief Convert node pointer to a string.
...
///  @return A string rep of a the node.
//------------------------------------------------------------------------------
        static std::string to_string(std::array<shared_leaf<T>, 4> branches) {
...
///  @param[in] nu_D_dt Normalized step rate.
//------------------------------------------------------------------------------
        apply_xi_node(shared_leaf<T> x,
                      shared_leaf<T> i,
                      shared_leaf<T> rand,
                      shared_leaf<T> nu_D_dt) :
        no_derivative<T, false,
                      n_branch_node<4, T, false>, 4> ({x, i, rand, nu_D_dt},
...
//------------------------------------------------------------------------------
        virtual void compile_preamble(std::ostringstream &stream,
                                      ...
                                      jit::preamble_map &pre_funcs,
                                      int &avail_const_mem) {
            if (!pre_funcs.contains("apply_xi")) {
                pre_funcs.insert("apply_xi");
...
//------------------------------------------------------------------------------
    template<std::floating_point T>
    shared_leaf<T> apply_xi(shared_leaf<T> x,
                            shared_leaf<T> i,
                            shared_leaf<T> rand,
                            shared_leaf<T> nu_D_dt) {
        auto temp = std::make_shared<apply_xi_node<T>> (x, i, rand,
                                                        nu_D_dt)->reduce();
//  Test for hash collisions.
        for (size_t i = temp->get_hash();
             i < std::numeric_limits<size_t>::max(); i++) {
...
//------------------------------------------------------------------------------
    template<std::floating_point T>
    shared_apply_xi<T> apply_xi_cast(shared_leaf<T> x) {
...
//------------------------------------------------------------------------------
///  @brief Get the characteristic volume.
///
///  @return l^3.
//------------------------------------------------------------------------------
        T get_volume() const {
...
//------------------------------------------------------------------------------
///  @brief Build a profile.
///
///  @param[in] func The profile function.
///  @returns The parallel temperature profile.
//------------------------------------------------------------------------------
        graph::shared_leaf<T> build_profile(std::function<graph::shared_leaf<T>(graph::shared_leaf<T>)> func) const {
            return func(x);
        }
    };
...
//------------------------------------------------------------------------------
///  @brief Model for coulomb scattering operators.
//------------------------------------------------------------------------------
    enum model {
...
//------------------------------------------------------------------------------
///  @brief Calculate ν_{E}.
///
///  @tparam T Base type of the calculation.
///  @tparam M The @ref pic::model of the calculation.
///
///  @param[in] xab
///  @param[in] mass_a   Mass of particle a.
///  @param[in] mass_b   Mass of particle b.
///  @param[in] gb
///  @param[in] nuab0
///  @param[in] erfp_xab Derivaive of error function.
///  @returns ν_{E}.
//------------------------------------------------------------------------------
    template<std::floating_point T, model M>
    graph::shared_leaf<T> build_colision_rate(graph::shared_leaf<T> xab,
                                              const T mass_a,
                                              const T mass_b,
                                              graph::shared_leaf<T> gb,
                                              graph::shared_leaf<T> nuab0,
                                              graph::shared_leaf<T> erfp_xab) {
        auto mass_ratio = static_cast<T> (2)*mass_a/mass_b*gb;
        auto nu = nuab0/xab;
        if constexpr (M == model::hinton) {
...
//------------------------------------------------------------------------------
///  @brief Build ion ion collision.
///
///  @tparam T Base type of the calculation.
///  @tparam M The @ref pic::model of the calculation.
///
///  @param[in]     ion_a         Ions for species a.
///  @param[in]     ion_b         Ions for species b.
///  @param[in]     mesh          A @ref pic::mesh object.
///  @param[in]     norms         A @ref pic::characteristics object.
///  @param[in]     params        A @ref pic::parameters object.
///  @param[in,out] total_density Accumulated density.
///  @param[in,out] total_flux    Accumulated total_flux.
///  @param[in]     state         Random state node.
//------------------------------------------------------------------------------
    template<std::floating_point T, model M>
    std::array<graph::shared_leaf<T>, 2> build_ion_ion_collision(const ion<T> &ion_a,
                                                                 const ion<T> &ion_b,
                                                                 const mesh<T> &mesh,
                                                                 const characteristics<T> &norms,
                                                                 const parameters<T> &params,
                                                                 graph::shared_leaf<T> total_density,
        ...
        auto tperp = ion_a.build_profile([](graph::shared_leaf<T> x) -> graph::shared_leaf<T> {
            return graph::one<T> ();
        });
        auto tb = static_cast<T> (0.5)*(tpara + tperp)*norms.te*kb<T>/q<T>;
        auto nv = ion_a.build_profile([](graph::shared_leaf<T> x) -> graph::shared_leaf<T> {
            return graph::one<T> ();
        ...
        return build_common_collision<T, M> (ion_a, norms, uxb, tb, nb, mass_b,
                                             zb2, dt, state);
    }

//------------------------------------------------------------------------------
///  @brief Build ion electron collision.
///
///  @tparam T Base type of the calculation.
///  @tparam M The @ref pic::model of the calculation.
///
...
///  @param[in] norms         A @ref pic::characteristics object.
///  @param[in] params        A @ref pic::parameters object.
///  @param[in] total_density Accumulated density.
///  @param[in] total_flux    Accumulated total_flux.
///  @param[in] state         Random state node.
//------------------------------------------------------------------------------
    template<std::floating_point T, model M>
    std::array<graph::shared_leaf<T>, 2> build_ion_electron_collision(const ion<T> &ion_a,
                                                                        const mesh<T> &mesh,
                                                                        const characteristics<T> &norms,
                                                                        const parameters<T> &params,
                                                                        graph::shared_leaf<T> total_density,
        ...
        auto uxb = total_flux/total_density;
...
//------------------------------------------------------------------------------
///  @brief Build common collisions graphs for all species.
///
///  @tparam T Base type of the calculation.
///  @tparam M The @ref pic::model of the calculation.
///
...
///  @param[in] norms  A @ref pic::characteristics object.
///  @param[in] uxb
///  @param[in] tb     Temperature of species b.
///  @param[in] nb     Density of species b.
///  @param[in] mass_b Mass of species b.
///  @param[in] zb2    Z effective squared.
///  @param[in] dt     Time step.
///  @param[in] state  Random state node.
//------------------------------------------------------------------------------
    template<std::floating_point T, model M>
    std::array<graph::shared_leaf<T>, 2> build_common_collision(const ion<T> &ion_a,
                                                                  const characteristics<T> &norms,
                                                                  graph::shared_leaf<T> uxb,
                                                                  graph::shared_leaf<T> tb,
                                                                  graph::shared_leaf<T> nb,
                                                                  const T mass_b,
                                                                  const uint8_t zb2,
                                                                  const T dt,
                                                                  const graph::shared_random_state<T> state) {
        const T mass_a = ion_a.mass*norms.m;
        const uint8_t za2 = ion_a.z*ion_a.z;

//  Convert to b frame.
        auto wxa = ion_a.v_para*norms.v - uxb;
        auto wya = ion_a.v_perp*norms.v;

//  Convert to sphereical.
        auto sphere = cartesian_to_sphereical(wxa, wya);

//  Apply Monte-Carlo collision operator.
        auto wtb = graph::sqrt(static_cast<T> (2)*q<T>*tb/mass_b);
        auto xab = sphere[0]/wtb;

        auto erf_xab = graph::erf(xab);
        auto erfp_xab = erf_xab->df(xab);
        auto erfpp_xab = erfp_xab->df(xab);

        auto gb = (erf_xab - xab*erfp_xab)/(static_cast<T> (2)*xab*xab);
        auto logA = static_cast<T> (30) - static_cast<T> (0.5)*graph::log(nb/(tb*graph::sqrt(tb)));
        auto nuab0 = nb*q<T>*q<T>*q<T>*q<T>*static_cast<T> (za2*zb2)*logA
                   / (static_cast<T> (2)*std::numbers::pi_v<T>*mass_a*mass_a*epsilon0<T>*epsilon0<T>*wtb*wtb*wtb);
...
//  Velocity Scattering operator.
        ...
        auto E_nuE_d_nu_E_dE = static_cast<T> (0.5)*((static_cast<T> (3.0)*(xab*erfp_xab - erf_xab) - xab*xab*erfpp_xab)/(erf_xab - xab*erfp_xab));

        const T mof = mass_a/(static_cast<T> (2)*q<T>);
        auto A = static_cast<T> (1) - static_cast<T> (2)*nu_e_dt;
        auto tbnu_e_df = tb*nu_e_dt;
        auto B = static_cast<T> (2)*tbnu_e_df*(static_cast<T> (1.5) + E_nuE_d_nu_E_dE);

        auto rand1 = graph::random<T> (state);
        auto u_op = graph::apply_u(sphere[0]*sphere[0], steps, rand1,
                                   graph::constant(mof), tbnu_e_df, A, B);
        ...
        sphere[1] = graph::apply_xi(sphere[1], steps, rand2, nu_D_dt);

        sphere[1] = graph::if_(sphere[1]*sphere[1] > static_cast<T> (1),
                               graph::copysign(static_cast<T> (1), sphere[1]) -
                               sphere[1] % graph::copysign(static_cast<T> (1), sphere[1]),
                               sphere[1]);

        auto cart = sphereical_to_cartesian(sphere[0], sphere[1], sphere[2]);
        return {(cart[0] + uxb)/norms.v, cart[1]/norms.v};
    }
...
//  Scale factor.
        ...
        return static_cast<T> (ion.z)*y*cf*sf;
    }

...
//------------------------------------------------------------------------------
        static size_t hash_data(const backend::buffer<T> &d) {
            ...
            for (size_t i = h; i < std::numeric_limits<size_t>::max(); i++) {
                if (leaf_node<T, SAFE_MATH>::caches.backend_cache.find(i) ==
                    leaf_node<T, SAFE_MATH>::caches.backend_cache.end()) {
                    leaf_node<T, SAFE_MATH>::caches.backend_cache[i] = d;
                    return i;
                } else if (d == leaf_node<T, SAFE_MATH>::caches.backend_cache[i]) {
                    return i;
                }
            }
...
//------------------------------------------------------------------------------
        virtual backend::buffer<T> evaluate() {
            return leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash];
        }
        ...
        virtual void compile_preamble(std::ostringstream &stream,
            ...
            if (visited.find(this) == visited.end()) {
                this->arg->compile_preamble(stream, registers,
                                            ...
                                            avail_const_mem);
                if (registers.find(leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash].data()) == registers.end()) {
                    registers[leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash].data()] =
                        jit::to_string('a', leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash].data());
                    const size_t length = leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash].size();
                    if constexpr (jit::use_metal<T> ()) {
                        textures1d.try_emplace(leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash].data(),
                                               length);
                        ...
                        stream << " " << registers[leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash].data()] << "[] = {";
                        if constexpr (jit::is_complex<T> ()) {
                            jit::add_type<T> (stream);
                        }
                        stream << leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash][0];
                        for (size_t i = 1; i < length; i++) {
                            stream << ", ";
                            if constexpr (jit::is_complex<T> ()) {
                                jit::add_type<T> (stream);
                            }
                            stream << leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash][i];
                        }
                        stream << "};" << std::endl;
                    }
                } else {
...
//------------------------------------------------------------------------------
        virtual bool has_constant_zero() const {
            return leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash].has_zero();
        }
...
//------------------------------------------------------------------------------
        size_t get_size() const {
            return leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash].size();
        }
    };
        ...
        virtual void compile_preamble(std::ostringstream &stream,
            ...
            if (visited.find(this) == visited.end()) {
                ...
                if (registers.find(leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash].data()) == registers.end()) {
                    ...
                    const size_t length = leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash].size();
                    if constexpr (jit::use_metal<T> ()) {
                        textures2d.try_emplace(leaf_node<T, SAFE_MATH>::caches.backend_cache[data_hash].data(),
